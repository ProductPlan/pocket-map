...
export interface MiniMapTarget {
  elementToMap: string; // The element that will be copied to create the mini-map
  coverWidthRef: string; // The element to use as a reference width when building the map (usually contains the elementToMap)
  widthRef: string; // TODO: don't think we need this anymore?
  scrollEl: string; // The element to watch for, and assign, scroll changes on/to
  readyIndicator: string; // An element, generally within the elementToMap, to use as a presence check before trying to build the map
}
...
export default function MiniMap({
  ...
  return (
    <div className={wrapperClasses}>
      {showMap && <Map target={mapTarget} mapType={MapType.relative} />}
      <div className="mini-map-controls">
        <a
          className={classNames("mini-map-control", controlClasses)}
          title={"Show minimap"}
          onClick={(e) => {
            e.preventDefault();
            setShowMap(!showMap);
        ...
        >
          {customIcon ? customIcon : <DefaultMapIcon />}
        </a>
      </div>
    </div>
  );
...
export const Map = ({
  ...
  useTargetScrollingToPositionViewport(
    target,
    draggingViewport,
    setReadyToMap,
    readyToMap
  ...
  useEffect(() => {
    if (readyToMap) drawMap(mapType, target);
  }, [readyToMap, mapType]);
  ...
  useEffect(() => {
    if (!readyToMap) checkIfReadyToMap(target.readyIndicator, setReadyToMap);
...
const positionMapFirstTime = (target: MiniMapTarget) => {
...
// based on class name.
const checkIfReadyToMap = (
  readyIndicator: string,
  setReadyToMap: (boolean) => void
) => {
  setTimeout(() => {
    const expectedElementsRendered =
      document.getElementsByClassName(readyIndicator).length > 0;
...
function useTargetScrollingToPositionViewport(
  target: MiniMapTarget,
...
const drawMap = (mapType: MapType, target: MiniMapTarget) => {
  Log.info("drawMap")
...
const drawRelativelySizedMap = (target: MiniMapTarget) => {
  const { clone, contentWidth, contentHeight } = createMappableClone(target);

  const getClone = () => clone as unknown as HTMLElement;
...
const listenToMiniMapEvents = (
  setDraggingViewport: (boolean) => void,
  target: MiniMapTarget
) => {
  ...
  map.addEventListener("mousedown", (e: React.MouseEvent<HTMLElement>) => {
    // Ignore click events on the viewport - they occur when dragging
    if (e.target === viewportElement || e.target === null) return;
  ...
  viewportElement.addEventListener(
    ...
    (e: React.MouseEvent<HTMLElement>) => {
      ...
      const wouldBeOutsideRight =
        scrollRef.scrollLeft + scrollRef.clientWidth + leftMoveDiff >
        (viewportElement.parentElement?.clientWidth || 1) * 10;
...
const Log = {
  warn: (message: any, ...arg) => console.warn("[MiniMap]", message, ...arg),
  info: (message: any, ...arg) => console.info("[MiniMap]", message, ...arg)
};
...
enum MapType {
  relative = 0, // The size of the minimap scales relative to the target
